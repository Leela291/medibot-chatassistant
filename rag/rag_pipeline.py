...
Fixed: Wikipedia query cleaning and topic validation.
"""
...
def run_rag(
    ...
    """
    Full RAG pipeline with fixed Wikipedia fallback and topic validation.
    """
    # 1. Retrieve relevant chunks from vector DB
    retrieved_chunks = retrieve(user_query, top_k=top_k)

    # 2. Smart Validation (Hard Mismatch Filter)
    ...
    if not retrieved_chunks:
        ...
        # Reject B: Chunk is about a disease not mentioned in the query
        elif chunk_disease_clean and chunk_disease_clean != "general":
            if chunk_disease_clean not in query_clean:
                print(f"[RAG Filter] Rejected '{chunk_disease}' context for query: '{user_query}'")
                needs_wikipedia = True
                retrieved_chunks = []

    # 3. Wikipedia Fallback — with proper query cleaning and validation
    wikipedia_context = ""
    if needs_wikipedia:
        try:
